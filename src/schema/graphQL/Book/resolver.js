const {
  ...
  dbHelpers,
  resolverHelpers
} = require("mongo-graphql-starter");
const hooksObj = require("../hooks");
...
const {
  getUpdateObject,
  setUpOneToManyRelationshipsForUpdate
} = updateUtilities;
const { ObjectId } = require("mongodb");
const BookMetadata = require("./Book");
...
module.exports = {
  Query: {
    async getBook(root, args, context, ast) {
      ...
      await runHook("queryMiddleware", queryPacket, {
        db,
        root,
        args,
        context,
    ...
    async allBooks(root, args, context, ast) {
      ...
      await runHook("queryPreprocess", { db, root, args, context, ast });
      context.__mongodb = db;
      let queryPacket = decontructGraphqlQuery(
        args,
        ast,
        BookMetadata,
        "Books"
  ...
  Mutation: {
    async createBook(root, args, context, ast) {
      ...
      return await resolverHelpers.runMutation(
        session,
        transaction,
        async () => {
          let newObject = await newObjectFromArgs(args.Book, BookMetadata, {
            ...gqlPacket,
            db,
            session
          });
          let requestMap = parseRequestedFields(ast, "Book");
          let $project = requestMap.size
            ? getMongoProjection(requestMap, BookMetadata, args)
            : null;
          ...
          await setUpOneToManyRelationships(
            newObject,
            args.Book,
            BookMetadata,
            { ...gqlPacket, db, session }
          );
          ...
          let result = $project
            ...
            : null;
          return resolverHelpers.mutationSuccessResult({
            Book: result,
            transaction,
            elapsedTime: 0
          });
    ...
    async updateBook(root, args, context, ast) {
      ...
      return await resolverHelpers.runMutation(
        ...
        async () => {
          ...
          let updates = await getUpdateObject(
            args.Updates || {},
            BookMetadata,
            { ...gqlPacket, db, session }
          );
          ...
          if (
            (await runHook("beforeUpdate", $match, updates, {
              ...gqlPacket,
              db,
              session
            })) === false
          ...
          await setUpOneToManyRelationshipsForUpdate(
            [args._id],
            args,
            BookMetadata,
            { ...gqlPacket, db, session }
          ...
          await runHook("afterUpdate", $match, updates, {
            ...gqlPacket,
            db,
            session
          });
    ...
    async updateBooksBulk(root, args, context, ast) {
      ...
      return await resolverHelpers.runMutation(
        ...
        transaction,
        async () => {
          let { $match } = decontructGraphqlQuery(
            args.Match,
            ast,
            BookMetadata
          );
          ...
          return await resolverHelpers.finishSuccessfulMutation(
            session,
            transaction
          );
        }
    ...
    async deleteBook(root, args, context, ast) {
      ...
      try {
        ...
        if (
          (await runHook("beforeDelete", $match, {
            ...gqlPacket,
            db,
            session
          })) === false
