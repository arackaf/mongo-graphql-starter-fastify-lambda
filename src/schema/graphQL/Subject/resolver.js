...
const SubjectMetadata = require("./Subject");
...
module.exports = {
  Query: {
    async getSubject(root, args, context, ast) {
      ...
      let results = await loadSubjects(
        db,
        aggregationPipeline,
        root,
        args,
    ...
    async allSubjects(root, args, context, ast) {
      ...
      if (queryPacket.$project) {
        result.Subjects = await loadSubjects(
          db,
          aggregationPipeline,
          root,
          args,
  ...
  Mutation: {
    async createSubject(root, args, context, ast) {
      ...
      return await resolverHelpers.runMutation(
        ...
        transaction,
        async () => {
          let newObject = await newObjectFromArgs(
            args.Subject,
            SubjectMetadata,
          ...
          let $project = requestMap.size
            ? getMongoProjection(requestMap, SubjectMetadata, args)
            : null;
